  ...
  bot.onText(/\/start(.*)/, async (msg, match) => {
    ...
    const welcomeText = isOwner(userId)
      ...
      : `👋 Hello ${msg.from.first_name}!\n\nTap below to browse all lectures! 📚` +
        (referLink
          ? `\n\n🔗 <b>Your Invite Link:</b>\n${referLink}\n\nShare with friends:\n• <b>You get:</b> +24h access & +1 Point 🎉\n• <b>They get:</b> 14h free access 🔓`
          : '');

    const _keyboard = [
      [{ text: "📚 Browse Lectures", web_app: { url: WEB_URL } }],
      ...(referLink ? [[{ text: "🔗 Invite Friends", url: referLink }]] : [])
    ];
    const _keyboardUrlOnly = [
      [{ text: "📚 Browse Lectures", url: WEB_URL }],
      ...(referLink ? [[{ text: "🔗 Invite Friends", url: referLink }]] : [])
    ];
    ...
    try {
      await bot.sendMessage(chatId, welcomeText, {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: _keyboard }
      });
    } catch (err) {
      console.error("/start web_app error, retrying with url button:", err.message);
      try {
        await bot.sendMessage(chatId, welcomeText, {
          parse_mode: 'HTML',
          reply_markup: { inline_keyboard: _keyboardUrlOnly }
        });
      } catch (err2) {
        bot.sendMessage(chatId, welcomeText.replace(/<[^>]*>/g, ''), {}).catch(() => {});
      }
    }
  });
