...
app.get("/api/config", (req, res) => {
  res.json({ ownerId: OWNER_ID, botUsername: BOT_USERNAME||"", forceJoinRequired: FORCE_JOIN_CHANNELS.length>0, contactLink: CONTACT_LINK||"https://t.me/FocusXSupport_Bot" });
});
    ...
    if (param) {
      ...
      try {
        ...
        if (isVideo && (record.delivered_to||[]).includes(chatId)) {
          const _dat = JSON.parse(record.delivered_at || '{}');
          const _lastTime = _dat[String(chatId)];
          const _sixHours = 6 * 60 * 60 * 1000;
          if (!_lastTime || (Date.now() - new Date(_lastTime).getTime()) < _sixHours) {
            return bot.sendMessage(chatId, `⚠️ This video was already delivered. You can request it again after 6 hours.`);
          }
          // 6h already passed but timer didn't fire (e.g. server restart) — clean up and allow
          FileRecord.updateOne({ _id: record._id }, { $pull: { delivered_to: chatId } }).catch(() => {});
          PendingUndeliver.deleteOne({ file_record_id: String(record._id), chat_id: chatId }).catch(() => {});
        }
        if (isVideo && !isOwner(userId)) {
          ...
          const _dat1 = JSON.parse(record.delivered_at || '{}'); _dat1[String(chatId)] = new Date().toISOString();
          FileRecord.updateOne({ code:record.code },{ $addToSet:{ delivered_to:chatId }, $set:{ delivered_at: JSON.stringify(_dat1) } }).catch(() => {});
        ...
        if (isVideo) {
          scheduleDelete(bot,chatId,sentMsg.message_id,new Date(Date.now()+6*60*60*1000));
          const _dat2 = JSON.parse(record.delivered_at || '{}'); _dat2[String(chatId)] = new Date().toISOString();
          FileRecord.updateOne({ code:record.code },{ $addToSet:{ delivered_to:chatId }, $set:{ delivered_at: JSON.stringify(_dat2) } }).catch(() => {});
          scheduleUndeliver(record._id, record.code, chatId, new Date(Date.now()+6*60*60*1000));
